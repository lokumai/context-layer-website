...
import { ChevronRight, ChevronDown, FilterX } from "lucide-react";
import { useState, useMemo } from "react";

type Severity = "critical" | "high" | "medium" | "low" | "info";

export function IntelligenceSecurity({ workspaceId: _workspaceId }: { workspaceId: string }) {
  ...
  const repoIds = useMemo(
    () =>
      security
        ? ["All", ...Array.from(new Set(security.findings.map((f) => f.repoId)))]
        : ["All"],
    [security],
  );
            ...
            {filteredFindings.map(finding => {
              ...
              return (
                <div key={finding.id} className="group">
                  ...
                  >
                    <div>
                      <StatusPill tone={getSeverityTone(finding.severity)}>{finding.severity}</StatusPill>
                    </div>
                    <div className="text-caption font-bold text-[#4e4e4e]">{finding.category}</div>
                    <div className="text-body-standard font-bold text-black pr-4 truncate">{finding.title}</div>
                  ...
                  {isExpanded && (
                    <div className="px-8 pb-6 pt-2 bg-[#fdfdfd] border-t border-[rgba(0,0,0,0.03)]">
                      <div className="text-micro font-bold text-[#777169] uppercase tracking-wider mb-2">Description</div>
